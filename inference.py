...
class SparseModel:
    def __init__(self, model_name="sentence-transformers/all-MiniLM-L6-v2"):
        ...
        self.transformer = Transformer(model_path, **sentence_bert_config)
    ...
    def encode_raw(self, sentences):
        if len(sentences) == 0:
            return [{"token_ids": [], "tokens": [], "weights": []}]
        ...
        attentions = (
            self.transformer.auto_model(**trans_features, output_attentions=True)
            .attentions[-1]
            .cpu()
        )
        ...
        for sentence_mask, token_ids, tokens, weight in zip(
            attention_mask, input_ids, text_tokens, weights
        ):
    ...
    def encode(self, sentences, as_tokens=False) -> Iterable[dict]:
        ...
        for i in range(len(sentences)):
            ...
            for j, token_id in enumerate(input_ids[i]):
                ...
                if token_weight > 0.0:
                    sentence_weights[token_id] = (
                        sentence_weights.get(token_id, 0) + token_weight
                    )

            yield sentence_weights

    def encode_documents(
        self, documents: List[List[str]], as_tokens=False
    ) -> Iterable[dict]:
        batch = []
        mapping = {}
        n = 0
...
def main():
    model = SparseModel()
    documents = ["I am a cat.", "Hello, I am a dog."]
